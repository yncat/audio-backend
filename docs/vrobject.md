...
## サンプルプログラムの変更
１つのVRObjectを用意する。 assets\gunloop.ogg をループサウンドに設定する。オブジェクトの center は width 3 height 0 depth 3 に設定。オブジェクトのサイズを 3, 3, 3 に設定。
まずは listener の位置を -1, 0, 0 に設定する。少しオブジェクトが右から聞こえてくるはず。
しばらくしたら、listener位置を 0, 0, 0 に設定する。これでオブジェクトの左の角に隣接するので、音が真ん中に来る。
しばらくしたら、listener位置を 3, 0, 0 に設定する。まだオブジェクトは中心から聞こえる。
しばらくしたら、listener位置を 4, 0, 0, に設定する。これで、オブジェクトは少し左から聞こえるようになる。
しばらくしたら、オブジェクトを破棄して終了。

# revision 4
サンプルプログラムのみの変更。
wide object test で、自動で次に進むようになっていた。これを、 press enter to continue という表示を出して、ユーザーがエンターを押したらlistener位置が変わるようにする。勝手に進むと、耳でデバッグするのが大変だった。
